...
parser.add_argument('--cnn-valid-err', dest='cnn_valid_err', type=int, default=40)
...
parser.add_argument('--rps-spike', dest='rps_spike', type=int, default=40,
...
DiurnalLoad = []
for u in range(MinUsers, MaxUsers + 1, UsersStep):
	DiurnalLoad.append((u, ExpTime))
for u in range(DiurnalLoad[-1][0] - UsersStep, MinUsers - 1, -UsersStep):
	DiurnalLoad.append((u, ExpTime))
print('Diurnal load pattern: ', DiurnalLoad)
...
def run_exp(diurnal_load, log_dir):
	global ViolTimeout
	global ExpTime
	...
	# execute diurnal load pattern
	exp_terminated = False
	service_fail = False
	prev_users = diurnal_load[0][0]
	for load_spec in diurnal_load:
		users = load_spec[0]
		exp_time = load_spec[1]
		# states_path 	= log_dir / 'states.txt'
		logging.info('\nTest users: ' + str(users))
		# normally create 1 new user each second
		locust_p = run_locust(
			client_script=LocustScript, csv=LocustCsv, 
			nginx_ip='http://127.0.0.1:8080', volumes=LocustVolumes,
			log_file=LocustLog, duration=exp_time + users,
			users=users, quiet=True)

		assert(locust_p != None)
		time.sleep(3)	
		# wait for all locust users to be active
		time_before_warmed = 3
		actual_users = 0
		while actual_users < min(users, prev_users):
			time_before_warmed += 1
			time.sleep(1)
			actual_users = get_locust_users(LocustStats)
			print('locust users %d all users %d prev users %d' %(actual_users, users, prev_users))
		prev_users = users
		...
		while time.time() - StartTime < exp_time + users - time_before_warmed:
			cur_time = time.time()
	...
	if not service_fail:
		save_states(log_dir)
		# copy_locust_stats(LocustStatsDir, sub_log_dir / 'locust_logs')
