...
int main(int argc, char **argv) {
    ...
    snprintf (pt_path, PATH_MAX, "%s/sinan-gcp/logs/pt.txt", home_dir);
    pt = fopen(pt_path, "w");
    // pt = fopen("/filer-01/yz2297/wrk2_log/pt.txt", "w");
    assert (pthread_mutex_init(&pt_lock, NULL) == 0);

    char per_resp_file_path[PATH_MAX];
    snprintf (per_resp_file_path, PATH_MAX, "%s/sinan-gcp/logs/per_resp.txt", home_dir);
    per_resp_file = fopen(per_resp_file_path, "w");
...
void *thread_main(void *arg) {
    ...
    snprintf (filename, PATH_MAX, "%s/sinan-gcp/logs/%" PRIu64 ".txt", home_dir, thread->tid);
    // char filename[50];
